...
contract DeFiFacet is Helper {
  event UniLeveraged(address indexed liquidityProvider, uint256 collateral, uint256 indexed healthFactor);
  ...
  function zappify(uint256 maxAmount) public payable returns (bool) {
    ...
    uint256 minAmt = msg.value / 5000000;
    ...
    // Adding Liquidity (min_liquidity,max_tokens,deadline)
    ...
    // Loan to value Ratio for borrowing dai is 75% so for safety keeping is as 70%
    uint256 daiToBorrow = (uniDai * 70) / 100;

    //Borrowing Safe Amount of DAI
    AaveInterface(getLendingPool()).borrow(getDai(), daiToBorrow, 1, 0);

    // Swapping 50 % DAI for ETH
    uint256 daiToSwap = daiToBorrow / 2;
    ...
    (, uint256 collateral, , , , , , uint256 healthFactor) = AaveInterface(getLendingPool()).getUserAccountData(msg.sender);
